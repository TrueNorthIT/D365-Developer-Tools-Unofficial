...
// ("System.IO.Packaging.Package" — the same container format as .docx/.xlsx) that Dataverse
// actually returns from RetrieveEntityRibbon. Shared by decompressRibbon.test.ts (unit-level) and
// ribbonIntegration.test.ts (full pipeline), so fixtures stay consistent with real payload shape
// without an archive library dependency.
